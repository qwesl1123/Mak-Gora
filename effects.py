...
EFFECT_TEMPLATES: Dict[str, Dict[str, Any]] = {
    ...
    "ancestral_guidance_shield": {
        "type": "status",
        "name": "Ancestral Guidance Shield",
        "duration": 2,
        "category": "absorb",
        "dispellable": True,
        "school": "magical",
    ...
    "frost_shock_freeze": {
        ...
        "tags": ["incapacitating_cc", "break_on_damage"],
        "cant_act_reason": "frozen",
        "break_on_damage_ignore_source_hit": True,
        "display": {
            "war_council": True,
            "label": "Frozen",
            "color": "#79C7FF",
...
def mitigation_effective_stat(
    ...
    *,
    ...
    if ignore_magic_resist:
        # ignore_magic_resist bypasses BOTH general Magic Resistance and the
        # matching subschool resistance (but never Defense).
        return max(0, defense)
    magic_resist = modify_stat(target, "magic_resist", target.stats.get("magic_resist", 0))
    # Matching magical subschool resistance is additive with general Magic
    # Resistance before the mitigation curve. The generic mapping owns the
    # subschool -> stat resolution: an unknown/missing subschool yields no
    # stat, and an absent resist stat defaults to zero, so callers without any
    # resistance behave exactly as before.
    resist_stat = subschool_resistance_stat(subschool)
    subschool_resist = (
        modify_stat(target, resist_stat, target.stats.get(resist_stat, 0))
        if resist_stat is not None
        else 0
    )
...
def trigger_on_hit_passives(
    ...
    for effect in attacker.effects:
        ...
        if passive_type == "burn":
            ...
            if raw <= 0:
                continue
            reduced = mitigate_damage(raw, target, "magic", subschool=passive.get("subschool"), challenger_mode=target_challenger_mode)
            ...
            for hit_index in range(1, hits + 1):
                ...
                duplicate_reduced = mitigate_damage(duplicate_raw, target, spell_school, subschool=spell_subschool, challenger_mode=target_challenger_mode)
...
def tick_dots(ps: PlayerState, log: List[str], label: str) -> list[dict[str, Any]]:
    ...
    for effect in ps.effects:
        ...
        if effect_id == "agony":
            ...
            reduced = mitigate_damage(raw_damage, ps, school, subschool=effect.get("subschool"))
