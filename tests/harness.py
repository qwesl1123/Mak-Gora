...
BoundedCombatLog = _MODS["models"].BoundedCombatLog
...
def _assert_invariants(match, prior_turn: int, prior_log_sequence: int) -> None:
    ...
    new_entry_count = match.log.sequence - prior_log_sequence
    new_lines = match.log[-new_entry_count:] if new_entry_count else []
...
def submit_turn(match, p1_ability_id, p2_ability_id):
    s1, s2 = match.players
    prior_turn, prior_log_sequence = match.turn, match.log.sequence
