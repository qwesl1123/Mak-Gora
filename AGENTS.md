...
Static detection has a floor: a determined author can still reach the filesystem through `importlib`, `sys.modules`, `eval`, a path assembled at runtime, or a value carried into a loop body, and no AST scan closes all of that. So the boundary is not left to static analysis alone — **`tests/run_architecture_guardrails.py` installs an audit hook that fails the run if any guardrail opens a `.md`/`.markdown` file**, before it imports the suite so import-time reads are covered too. That observes the real invariant rather than approximating it, and it catches every route source inspection cannot follow. The static checks remain because they name the problem precisely and catch it at review time; the hook is what makes them sufficient rather than exhaustive. `guardrail_markdown_reference_detector_self_test` pins the hook's presence and its position before the import, so neither can be dropped silently.

The authoritative end-to-end check is still behavioral: **run the standard suites against a nested tree containing no Markdown at all** before merging anything that touches test path discovery. The literal scan has **no exemptions**. Every Markdown filename the suite needs as detector input is assembled by `_markdown_fixture_name()` rather than written out, so the module contains no Markdown path literal at all and the scan can be absolute: any such literal is a failure, full stop. **Do not write a Markdown filename literally in this suite** — build it.

Assembled names are invisible to a literal scan, so the fixtures are governed by their *uses* instead. A reference to `_MARKDOWN_FIXTURE_NAMES` must satisfy **both** conditions: it sits in a module-level fixture definition or a function in `_MARKDOWN_FIXTURE_CONSUMERS`, **and** its use is one of the detector-only shapes — iteration, `len()`, `str.join()`, `str.endswith()`. Location alone is not enough: a fixture handed to `_read(...)` inside an allowed consumer is still a documentation dependency. `_markdown_fixture_name()` consequently has no legal use inside any function body — assembling the tables at module level is its only purpose, and a guardrail has no business building a Markdown name. Extend the consumer set or the permitted shapes only for something that genuinely just feeds the detector.
