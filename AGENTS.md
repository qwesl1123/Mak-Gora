...
* Resolve repository documentation with `path_discovery.detect_repository_root()`; never derive it from the template directory (`_detect_duel_html_path().parent / "AGENTS.md"` is prohibited), and never copy documentation into `templates/`. Root discovery is scoped to the single root its layout allows — the package root for a flat checkout, the application root two levels above a `games/duel` package — and raises rather than searching further up, so architecture validation can never silently validate an unrelated checkout that happens to carry documentation.
