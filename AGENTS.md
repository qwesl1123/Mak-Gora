...
* Broad modifiers are separate systems and must not be folded into `empowered_by`: Flame Dance (next qualifying Fire ability, 1.5x raw), Avenging Wrath's global `outgoing_damage_mult` on its effect template, Onslaught stacks, generic `empower_next_offense` effects, and Clarity of Mind (one stack consumed per Flash Heal / Penance / Penance (Self) cast at the cast-commit boundary — even when the cast misses or the target is immune — multiplying that cast's final damage/healing by 1.4 and logging "Empowered by Clarity of Mind!" once per consuming cast).
