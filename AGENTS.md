...
Assembled names are invisible to a literal scan, so the fixtures are governed by their *consumers* instead: `_MARKDOWN_FIXTURE_NAMES` may only be referenced from the closed set in `_MARKDOWN_FIXTURE_CONSUMERS` (plus the fixture definitions themselves). That is what stops a fixture value from being fed to a file read — the one route by which an assembled name could still become a dependency. Only extend that consumer set for something that genuinely just feeds the detector.
